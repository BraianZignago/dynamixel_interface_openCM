...
def apply_pwm(DXL_ID, goal):
    ...
    # Write goal position
    if (MY_DXL == '2XL430'): # XL320 uses 2 byte Position Data, Check the size of data in your DYNAMIXEL's control table
...
#    apply_pwm(DXL_ID_4, data.data[3])
    mandar = data.data[0] + ',' + data.data[1] + ',' + data.data[2] + ',' + data.data[3]
    print(mandar)
